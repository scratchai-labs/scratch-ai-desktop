...
import type { LoadedServerConfig } from "./server-config";
...
export class CoachService {
  ...
  async generateHint(options: GenerateCoachHintOptions): Promise<GenerateCoachHintResult> {
    ...
    try {
      const coachResponse = await this.requestServerHint(normalizedOptions);
      return {
        source: "server",
  ...
  private async requestServerHint(options: GenerateCoachHintOptions) {
    ...
    try {
      const releaseId = options.aiConfig.releaseId;
      const accessToken = options.aiConfig.apiKey;
      if (!releaseId || !accessToken) {
        throw new Error("联机配置不完整，请先保存服务器地址、学生 Token 和 Release ID。");
      }

      const fallbackCoachResponse = buildFallbackCoachResponse(options);
      const progressResponse = await this.fetchImpl(`${options.aiConfig.baseUrl}/api/student/releases/${releaseId}/progress`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          currentTarget: options.snapshot.currentTarget || "当前角色",
          stepSummary: fallbackCoachResponse.nextStep,
          snapshot: buildPromptContext(options)
        }),
      ...
      if (!progressResponse.ok) {
        ...
        throw new Error(`服务器进度上报失败：${progressResponse.status} ${responseText.slice(0, 240)}`);
      }

      const hintResponse = await this.fetchImpl(`${options.aiConfig.baseUrl}/api/student/releases/${releaseId}/hints`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${accessToken}`
      ...
      if (!hintResponse.ok) {
        const responseText = await hintResponse.text();
        throw new Error(`服务器提示请求失败：${hintResponse.status} ${responseText.slice(0, 240)}`);
      }

      const rawPayload = await hintResponse.json();
      const answerText = normalizeTextValue((rawPayload as Record<string, unknown>).prompt);
      if (!answerText) {
        throw new Error("服务器提示接口没有返回可用的 prompt。");
      }
