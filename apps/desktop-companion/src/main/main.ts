...
async function handleSaveServerConnection(config: { baseUrl: string; accessToken: string; releaseId: string }) {
  if (launchOptions.automationActions) {
    return;
  }
  await sessionManager?.saveServerConnection(config);
}

async function handleClearServerConnection() {
  if (launchOptions.automationActions) {
    return;
  }
  await sessionManager?.clearServerConnection();
}
...
ipcMain.handle("desktop-companion:save-server-connection", async (_event, config: {
  baseUrl: string;
  accessToken: string;
  releaseId: string;
}) => {
  await handleSaveServerConnection(config);
});
ipcMain.handle("desktop-companion:clear-server-connection", async () => {
  await handleClearServerConnection();
});
