...
interface SessionManagerDependencies {
  ...
  loadAiConfig?: typeof loadServerConfig;
  platform?: string;
...
export class SessionManager {
  ...
  private readonly loadAiConfig: typeof loadServerConfig;
  ...
  private aiConfig: LoadedServerConfig | null = null;
  ...
  constructor(
    ...
    this.loadAiConfig = dependencies.loadAiConfig ?? loadServerConfig;
  ...
  async saveServerConnection(config: { baseUrl: string; accessToken: string; releaseId: string }) {
    this.config = await this.configStore.saveServerConnection(config);
    await this.refreshAiConfig();
  ...
  async clearServerConnection() {
    this.config = await this.configStore.clearServerConnection();
    await this.refreshAiConfig();
    this.stateStore.update({
      ...this.getAiStatePatch(),
      aiError: undefined
  ...
  async requestAiHint(goal?: string) {
    ...
    if (result.warning) {
      this.log("Server live hint fell back to local heuristics", result.warning);
    }
  ...
  private setWaitingState(detail?: string) {
    ...
    const nextState: DesktopCompanionState = {
      ...
      aiCustomModelConfigured: Boolean(trimText(this.config.serverReleaseId)),
      aiCustomPromptConfigured: false,
  ...
  private async refreshAiConfig() {
    this.aiConfig = await this.loadAiConfig(undefined, {
      ...
      accessToken: this.config.serverAccessToken,
      releaseId: this.config.serverReleaseId,
  ...
  private getAiStatePatch() {
    return {
      ...
      aiCustomKeyConfigured: this.aiConfig?.customKeyConfigured ?? false,
      aiCustomModelConfigured: Boolean(trimText(this.config.serverReleaseId)),
      aiCustomModel: this.config.serverReleaseId,
      aiCustomPromptConfigured: false,
