...
export function formatAiStatus(state: DesktopCompanionState) {
  ...
  if (state.aiCoachResponse && state.aiProvider === "server") {
    const modelText = state.aiModel ? `（${state.aiModel}）` : "";
    return `当前提示来源：服务器${modelText}，生成时间：${formatTimestamp(state.aiLastUpdatedAt)}`;
  }

  if (state.aiCoachResponse && state.aiProvider === "fallback") {
    return state.aiError
      ? "服务器暂时不可用，已自动切到基础提示。"
      : "当前提示来源：基础提示。需要更完整结果时，可到“联机设置”里保存服务器配置。";
  }
  ...
  if (!state.aiConfigured) {
    return isManualHintTriggerMode(state)
      ...
      : "还没配置联机参数也可以先用。自动模式下，程序会先给基础提示；需要更完整结果时，再到“联机设置”里保存服务器配置。";
  }
