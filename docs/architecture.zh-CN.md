...
当前仓库已经整理成独立的 npm workspace，只保留联机桌面客户端主线。
...
12. 用户请求 AI 提示时，桌面端先向服务器上报当前进度，再读取服务器返回的提示
13. 当服务器不可用或配置不完整时，`CoachService` 自动回退到本地 fallback
